class Diana{
    ...
    draw(){
        ...
        let cosa = 30*(camWidth/2 - FACE_X)/(camWidth/2);
        while(r >= this.arcDist){
            push();
            translate(this.pos.x + /*Math.sin(frameCount*0.02)*60*/cosa*((this.rad-r)/this.rad), this.pos.y);
            let offset = 0;
            if(colIndex){
        ...
        fill(0);
        ellipse(this.pos.x + /*sin(frameCount*0.02)*60*/cosa, this.pos.y, this.arcDist/2, (this.arcDist/2) - this.arcDist*Math.abs((t/p) - Math.floor((t/p) + 0.5)));
    }
}